...
class Single_Pass_Cluster(object):
    ...
    def fit_transform(self, theta):
        ...
        for k in clusterTopic_list:
            ...
            # print(
            #     "主题文档数】：{} \n【主题关键词】：{} \n【主题中心句】 ：\n{} \n 链接】 ：\n{}".format(len(k[1]), keywords, keysentences, urls_to_print))
            # print("-------------------------------------------------------------------------")
            # if len(k[1]) >= 2:
            self.result.append({
    ...
    def check_similarity(self, original, latest, delta):
        ...
        """
        intersection = []
        old = original[:]  # 这里进行列表复制是防止下面对数据的修改影响到原数据，因此取交并集的部分用索引定位原数据
        new = latest[:]
        for i_idx, i in enumerate(new):
            flag = 0  # 用于标记是否找到相似内容
            for j_idx, j in enumerate(old):
                if j == 0:  # 此旧话题已合并过相似的新话题，可以跳过
                    continue
...
if __name__ == '__main__':
    ...
    crawler_col = db['wangyi']
    crawler_col2 = db['red_news']
    ...
    start = end - delta
    # end = start + delta
    result1 = list(crawler_col.find({
        'hotspot_data.time': {
            '$gt': start.strftime('%Y-%m-%d %H:%M:%S'),
            '$lt': end.strftime('%Y-%m-%d %H:%M:%S')
        }
    }))
    result2 = list(crawler_col2.find({
        'hotspot_data.time': {
            '$gt': start.strftime('%Y-%m-%d %H:%M:%S'),
            '$lt': end.strftime('%Y-%m-%d %H:%M:%S')
        }
    ...
    a = Single_Pass_Cluster(origin_data=result1, stop_words_file='停用词汇总.txt', generate_time=end.strftime('%Y-%m-%d %H:%M:%S'))
    result = a.fit_transform(theta=theta)
    # print(result)
